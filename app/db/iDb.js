//database gateway
//Uses the steganographer
const events = require('events');
const Steganographer = require('../s/steganographer');
...
/**
 * Exported class, gives a chance to set the image
 * @param {String} pathToImage - The path to the image file to be used as database
 */
  ...
  connect(){
   if(this.pathToImage){
    ...
    //Check File Access and Existence
      fs.access(this.pathToImage,fs.constants.R_OK && fs.constants.W_OK,(err)=>{
        if(err){
          if(err.code === 'ENOENT'){
        ...
        //???? add check if it is an image file
        let s = new Steganographer(this.pathToImage);

        s.ready((sInstance)=>{
          this.emit('ready',new iDb(sInstance));
        });
...
class iDb{
  ...
  init(){
   //get the raw data from the steganographer, this is the Object that iDb will use as database 
   ...
   if(collectionNames.length > 0){
     collectionNames.forEach((collectionName)=>{
      if(self[collectionName] === undefined){//if the collection name is not yet a property of iDb
       //define each collection names as property of iDb so that iDb.collectionName can be used
  ...
  createCollection(collectionName){
   if(this.sData[collectionName]){
    ...
    //define collection name as property of iDb
...
class Collection{
  ...
  constructor(name,arr = [],iDb){
    ...
    Object.defineProperty(this,'iDb',{value: iDb, enumerable:false, writable:false});
    Object.defineProperty(this,'insert',{value: iDb.insert.bind(iDb,this)});//use iDb's insert,curry in the collection
    Object.defineProperty(this,'selectAll',{value: iDb.selectAll.bind(iDb,this)});
    Object.defineProperty(this,'remove',{value: iDb.removeCollection.bind(iDb,this)});
    Object.defineProperty(this,'find',{value: iDb.find.bind(iDb,this)});
    Object.defineProperty(this,'drop',{value: iDb.drop.bind(iDb,this)});
    Object.defineProperty(this,'updateOne',{value: iDb.updateOne.bind(iDb,this)});
    Object.defineProperty(this,'deleteOne',{value: iDb.deleteOne.bind(iDb,this)});
  }
}
